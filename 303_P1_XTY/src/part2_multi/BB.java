...
public class BB<E> {
	 ...
	 *
	 */
	public BulletinItem post(E item, Type type, User auth, int uses) throws InterruptedException{
		if (item == null){
			return null;
		}
		BulletinItem object = new BulletinItem(item, type, auth, uses);
		this.add(object);
		return object;
	}
	...
	public class BulletinItem implements Comparable<BulletinItem>{
		...
		public BulletinItem(E value, Type type, User auth, int uses){
			this.value = value;
			this.type = type;
			this.auth = auth;
			available = new AtomicInteger(uses);
		}
		
		//Requests success of failure
		public boolean request() throws InterruptedException{
			int remaining = available.getAndDecrement();
