 ...
 * 
 * This currently requests integers
 */
public class Client extends Thread implements User{
	...
	private void postBulletin() throws InterruptedException{
		post.compareAndSet(null, bulletin.post(request.get(), Type.REQUESTED, this, 1));
	}
	...
	public boolean match(User user, BB<Integer>.BulletinItem item) throws InterruptedException {
		...
		if (item == null || post.get() != item){
			return false;
		}
		boolean success = post.compareAndSet(item, null);		
		//Match successful
		if (success){
			if (user != this){System.out.println(this.getName()+ " matched with "+((Thread)user).getName());}
			request.set(null);
